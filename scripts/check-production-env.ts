/**
 * Prebuild gate (2026-09-25, controller Change 3). npm runs `prebuild` before `build`
 * automatically, so this runs ahead of every `next build`, including on Vercel.
 *
 ...
 *
 * Run: node --experimental-strip-types --disable-warning=ExperimentalWarning scripts/check-production-env.ts
 */
