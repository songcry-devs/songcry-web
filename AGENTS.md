...
- `npm test` and `npm run build` must pass before you open a PR (`npm test` needs Node 22.18 or later). A build with `VERCEL_ENV=production` refuses to finish unless `OUTREACH_SUPABASE_URL`, `OUTREACH_SUPABASE_ANON_KEY` and `FAN_WAITLIST_URL` are set and clean (`scripts/check-production-env.ts`); that is deliberate.
